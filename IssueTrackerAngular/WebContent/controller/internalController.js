(function() {
	...
	angular.module('issueTracker').controller('InternalController',
			InternalController);

	InternalController.$inject = [ '$state', 'HTTPService', '$scope',
			'$uibModal', 'SelectHelper','ngDialog' ];
	function InternalController($state, HTTPService, $scope, $uibModal,
			SelectHelper,ngDialog) {
		console.log('InternalController');
		$scope.status1 = SelectHelper.getStatus();
		$scope.team1 = SelectHelper.getTeam();
		var trackerId = 4; //For Internal Tracker
		var issues = {};
		$scope.gridOptions = {
